    ...
    <div class="row g-4">
        <div class="col-lg-4">
            <section class="detail-panel p-4 h-100">
                ...
                <div class="detail-row"><span class="text-muted">Jadwal</span><span class="text-end">{{ optional($attendance->workSchedule)->name ?: $attendance->employee->workSchedule->name }}</span></div>
                <div class="detail-row"><span class="text-muted">Lokasi</span><span class="text-end">{{ $attendance->location->name }}</span></div>
            </section>
        </div>
                ...
                @foreach ([
                    ...
                    <div class="col-md-6">
                        <section class="detail-panel p-4 h-100">
                            ...
                            <div class="detail-row"><span class="text-muted">Waktu</span><strong>{{ $time ? $time->format('H:i:s') : '-' }}</strong></div>
                    ...
                    @foreach ($attendance->corrections as $correction)
                        <div class="border-bottom py-3">
                            ...
                            <div class="small mb-1">
                                Masuk: {{ $correction->old_check_in ? $correction->old_check_in->format('H:i') : '-' }} → {{ $correction->new_check_in ? $correction->new_check_in->format('H:i') : '-' }}
                            </div>
                            <div class="small mb-2">
                                Pulang: {{ $correction->old_check_out ? $correction->old_check_out->format('H:i') : '-' }} → {{ $correction->new_check_out ? $correction->new_check_out->format('H:i') : '-' }}
                            </div>
