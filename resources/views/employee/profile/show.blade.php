...
<style>
    ...
    .employee-profile-avatar { width:88px; height:88px; display:grid; place-items:center; margin:auto; color:#174f9f; background:#eef5ff; border:1px solid #d9e6f8; border-radius:50%; }
...
<section class="employee-profile">
    <header class="employee-profile-head"><div class="employee-profile-avatar mb-3"><x-icon name="user" size="42" /></div><h2 class="h4 fw-semibold mb-1">{{ $user->name }}</h2><div class="text-muted">{{ $user->employee->employee_number }} · {{ $user->employee->position ?: 'Pegawai' }}</div></header>
    <div class="px-4 pb-3">
        ...
        <div class="employee-profile-row"><span class="text-muted">Jadwal dasar</span><span class="text-end">{{ $user->employee->workSchedule->name }}</span></div>
    ...
    <div class="border-top px-4 py-3"><div class="small fw-bold text-muted text-uppercase mb-2">Pengaturan Akun</div>
        <a class="employee-profile-link" href="{{ route('employee.profile.edit') }}"><x-icon name="edit" /><span><strong class="d-block">Edit Informasi Akun</strong><small class="text-muted">Ubah nama, username, email, dan telepon.</small></span><x-icon name="chevron" class="ms-auto" /></a>
