...
(async () => {
    ...
    try {
        ...
        assert.equal(await page.locator('.app-shell').evaluate(node => node.classList.contains('sidebar-open')), true);
        await page.keyboard.press('Escape');
        assert.equal(await page.locator('.app-shell').evaluate(node => node.classList.contains('sidebar-open')), false);
        assert.equal(await page.locator('#sidebarToggle').evaluate(node => document.activeElement === node), true);

        const validHtml = html;
        html = html.replace(/data-schedule='[^']*'/, "data-schedule='{invalid}'");
        await page.goto('http://dashboard.test/dashboard', { waitUntil: 'networkidle' });
        assert.equal(await page.locator('#employeeScheduleTicker').getAttribute('data-schedule-error'), 'true');
        assert.match(await page.locator('#employeeScheduleTicker p').textContent(), /belum dapat dimuat/i);
        html = validHtml;
        await page.goto('http://dashboard.test/dashboard', { waitUntil: 'networkidle' });
        ...
        assert.equal(await page.locator('#checkoutModal').evaluate(node => node.classList.contains('is-early')), true);
        assert.equal(await page.locator('#checkoutModal .checkout-modal__description').evaluate(node => getComputedStyle(node).backgroundColor), 'rgb(255, 251, 235)');
        assert.equal(await page.locator('#checkoutModal .checkout-modal__icon').count(), 1);
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), true);
        await page.screenshot({ path: path.join(output, 'checkout-modal-390.png'), fullPage: true });
        ...
        await page.locator('#checkOutCard').click();
        await page.locator('#checkoutModal.show').waitFor();
        assert.equal(await page.locator('#checkoutModal .modal-dialog').evaluate(node => node.classList.contains('modal-dialog-scrollable')), true);
        assert.equal(await page.locator('#checkoutModal .modal-content').evaluate(node => node.getBoundingClientRect().bottom <= innerHeight + 1), true);
        await page.locator('#earlyCheckoutReason').fill('Keperluan keluarga');
        assert.equal(await page.locator('#earlyReasonCounter').textContent(), '18/1000');
        await page.screenshot({ path: path.join(output, 'checkout-modal-360x640.png') });
        await page.getByRole('button', { name: 'Batal', exact: true }).click();
        await page.setViewportSize({ width: 1440, height: 1000 });
        await page.goto('http://dashboard.test/dashboard', { waitUntil: 'networkidle' });
        await page.locator('#checkOutCard').click();
        await page.locator('#checkoutModal.show').waitFor();
        await page.screenshot({ path: path.join(output, 'checkout-modal-1440.png'), fullPage: true });
        await page.getByRole('button', { name: 'Batal', exact: true }).click();
        html = render('admin');
        await page.goto('http://dashboard.test/dashboard', { waitUntil: 'networkidle' });
